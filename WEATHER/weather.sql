...
-- adding timestamps to assist with deletions (ship table had no
-- deletions, worked up to over 700K entries!)

CREATE TABLE buoy (  
STN, LAT, LON, YYYY, MM, DD, hh, minute, WDIR, WSPD, GST, WVHT, DPD, APD, 
MWD, PRES, PTDY, ATMP, WTMP, DEWP, VIS, TIDE, raw_text DEFAULT '',
...
CREATE TABLE metar ( 
raw_text DEFAULT '', station_id, observation_time, latitude,
longitude, temp_c, dewpoint_c, wind_dir_degrees, wind_speed_kt,
...
metar_type, elevation_m, timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
sea_level_pressure_mb, station_id, temp_c, wind, raw_text DEFAULT '',
timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP  
);
