...
struct option options[] = {
 ...
 char *frames[5] = {"J2000", "ECLIPJ2000", "EQEQDATE", "ECLIPDATE", "ALTAZ"};
 ...
 for (double i = start; i <= end; i += delta) {
   ...
   // print out time and id at start of each line

   printf("%d %f ", id, i); 

   // HACK: 8 is size of char *, but how would I know that?

   for (int j=0; j < sizeof(frames)/8; j++) {

     strcpy(frame, frames[j]);
     ...
     printf("%s %f %f ", frame, ra*dpr_c(), dec*dpr_c());
   }
   printf("\n");
 }
