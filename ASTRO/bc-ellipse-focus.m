...
(* theta is accurate w/ respect to coordinates, theta2 is easier to invert *)
(* theta[t_] = ArcTan[x[t]-focus[a,b], y[t]] *)
theta[t_] = ArcTan[y[t]/(x[t]-focus[a,b])]

(* testing again *)
a = 1.1; b = 1;
Plot[{theta[t],theta2[t]},{t,0,Pi/2}]
Unset[a]; Unset[b];

t[theta_] = InverseFunction[theta2][theta]
...
r2[theta_] = FullSimplify[x[theta]^2 + y[theta]^2]
