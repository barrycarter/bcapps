...
NDSolve[{
 t0 = 2456658.500000000;
 g = 6.67384*10^-11;
 mm = 328.5*10^21;
 ms = 1.989810^30;
 d2[t_] = mx[t]^2 + my[t]^2 + mz[t]^2;
 pull = g*ms;
 mx[t0] == 1.207138059004780*10^-01,
 my[t0] == -4.365387430471273*10^-01,
 mz[t0] == -4.656000296377556*10^-02,
 mx'[t0] == 2.148807180306500*10^-02,
 my'[t0] ==  8.914250181959474*10^-03,
 mz'[t0] == -1.242939299365231*10^-03,
 mx''[t] == -pull/d2[t],
 my''[t] == -pull/d2[t],
 mz''[t] == -pull/d2[t]
}, {mx,my,mz}, {t, t0, t0+3650}]

x[t_] = mx[t] /. %[[1]]
Plot[x[t],{t,t0,t0+365}]
