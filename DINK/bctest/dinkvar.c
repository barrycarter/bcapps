...
void load_map_to(char* path, const int num, struct small_map* screen)
{
  ...
  // putting all BC comments here to minimize diffs:
  // TODO: this needs to be more portable
  // save_game(0) will eventually be read by bc-make-screen.pl
  // variables like lsize and holdme are no longer used, since only
  // first 31280 bytes of created map.dat are read
  save_game(0);
  char cmd[200];
  sprintf(cmd,"/home/barrycarter/BCGIT/DINK/bc-make-screen.pl %d %s",num,paths_dmodfile(path));
  system(cmd);
