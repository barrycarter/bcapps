...
($out,$err,$res) = cache_command2("curl --trace-ascii - -H 'Referer: http://chat.stackexchange.com/rooms/$rid' -H 'X-Requested-With: XMLHttpRequest' -H 'User-Agent: Fauxzilla' -b 'x=0' -d 'fkey=$fkey&r$rid=$ts' http://chat.stackexchange.com/events", "age=3600");
